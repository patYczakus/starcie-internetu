...
const spf = {
    ...
    theDarkLord: function() {
        ...
        gameModify.spSounds.magic.play()
        setTimeout(() => {
            for (var i = 0; i < conf.maxS * conf.atkPerSec; i++) setTimeout(() => {
    ...
    kiranaYonome: function() {
        get(ref(getDatabase(app), `starcie-internetu/followersApiInfo/ky`)).then(snpsht => {
            gameModify.getColab().enemy.attack(gameModify.calc(0, snpsht.val() / (Math.round(Math.random() * 30) + 1), 3.2, gameModify.getColab().you.getLevel()))
            gameModify.spSounds.uAttack.currentTime = 0
            gameModify.spSounds.uAttack.play()
            gameModify.getColab().endSP()
        })
    ...
    snackowyAdmin: function() {
        ...
        if (gameModify.getColab().enemy.name() != "paty" || rand) {
            ...
            if ("heal" in snacksInfo[acctualNum]) {
                gameModify.getColab().you.hp.setValue(gameModify.getColab().you.hp.get() + gameModify.calc(0, snacksInfo[acctualNum].heal[0], snacksInfo[acctualNum].heal[1], gameModify.getColab().you.getLevel()), false)
            }
            ...
            if ("def" in snacksInfo[acctualNum]) {
                JSONN.critChance = gameModify.getColab().you.JSON.get("critChance") - gameModify.calc(1, snacksInfo[acctualNum].def[0], snacksInfo[acctualNum].def[1], gameModify.getColab().you.getLevel())
            }
